...
def BYTE(line: str) -> list:
    ...
    if mode == 'C':
        obj += ''.join([hex(ord(c))[2:] for c in data]) ## 字符串轉換成十六進制
    elif mode == 'X':
        obj += data
    else:
        return 0, 'ERROR'
    
    return len(obj) // 2, obj #返回位元組數目標碼
...
def RESW(line: str) -> int:
    return int(line) * 3 #返回保留的字數量，每字佔3位元組
...
def SICXE_Assambler() -> None:
    ...
    setLocation() #設定位置並將相關資訊存儲在全域變數中

    print('\n'.join(ADDR_LIST)) #位置的清單
